...
const Sidebar = ({}) => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-while ${selectedUser ? "max-md:hidden" : ''}`}>
      <div className='pb-5'>
    ...
    <div className='flex flex-col'>
        {filteredUsers.map((user, index)=>(
            <div onClick={()=>{setSelectedUser(user); setUnseenMessages(prev=>({...prev, [user._id]:0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm text-white ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
